...
public class MSGHelpCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!(sender instanceof Player)) {
            ...
            String consoleHelpList = ColorUtils.translateColorCodes(OgerLabs.getInstance().getConfig().getString("helpmessages.consolehelplist"));
            plugin.getServer().getLogger().info(consoleHelpList);
        } else {
            String playerHelpList = ColorUtils.translateColorCodes(OgerLabs.getInstance().getConfig().getString("helpmessages.playerhelplist"));
