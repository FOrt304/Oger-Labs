...
public class PlayerMsgCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        try {
            ...
            if (Objects.equals(blockedSender.getUniqueId().toString(), uuidArgPlayer)) {
                if (!OgerLabs.getInstance().getConfig().getBoolean("sendmsgyourself")) {
                    sender.sendMessage(ColorUtils.translateColorCodes(OgerLabs.getInstance().getConfig().getString("messages.notmsgyouself")));
                    return true;
                }
            }
            ...
            for (Map<String, Object> i : rsBlockFirst) {
                if (Objects.equals(i.get("UUID"), uuidArgPlayer)) {
                    sender.sendMessage(ColorUtils.translateColorCodes(OgerLabs.getInstance().getConfig().getString("messages.youinbl")));
                    return true;
                }
            }
            ...
            for (Map<String, Object> i : rsBlockSecond) {
                if (Objects.equals(i.get("BlockedUUID"), uuidArgPlayer)) {
                    sender.sendMessage(ColorUtils.translateColorCodes(OgerLabs.getInstance().getConfig().getString("messages.youbl")));
                    return true;
                }
            }
            ...
            String msgSenderPattern = ColorUtils.translateColorCodes(OgerLabs.getInstance().getConfig().getString("messages.msgsenderpattern"));
            ...
            String msgRecipientPattern = ColorUtils.translateColorCodes(OgerLabs.getInstance().getConfig().getString("messages.msgreceiverpattern"));
    ...
    private void sendOfflineMessage(CommandSender sender, UUID uuid, String playerName, String message) {
        OgerLabs.getInstance().offlineReceiver.put(uuid, playerName);
        OgerLabs.getInstance().offlineMessages.put(uuid, message);

        sender.sendMessage(ColorUtils.translateColorCodes(OgerLabs.getInstance().getConfig().getString("messages.playermissing")));
        sender.sendMessage(ColorUtils.translateColorCodes(OgerLabs.getInstance().getConfig().getString("messages.msgsendoffline")));
        TextComponent msgSendOffline = new TextComponent(OgerLabs.getInstance().getConfig().getString("messages.acceptsend"));
        msgSendOffline.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text(OgerLabs.getInstance().getConfig().getString("messages.clickmsgsendoffline"))));
        msgSendOffline.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/acceptsend"));
        sender.sendMessage(String.valueOf(msgSendOffline));
        ...
        Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() {
            ...
            public void run() {
                if (OgerLabs.getInstance().offlineReceiver.containsKey(uuid) && OgerLabs.getInstance().offlineMessages.containsKey(uuid)) {
                    OgerLabs.getInstance().offlineReceiver.remove(uuid, playerName);
                    OgerLabs.getInstance().offlineMessages.remove(uuid, message);
                }
            }
        }, 1200);
    }
