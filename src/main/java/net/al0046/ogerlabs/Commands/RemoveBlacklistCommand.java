...
public class RemoveBlacklistCommand implements CommandExecutor {
    ...
    public boolean onCommand( CommandSender sender, Command command, String label, String[] args) {
        ...
        try {
            ...
            if (rs.isEmpty()) {
                sender.sendMessage(ColorUtils.translateColorCodes(OgerLabs.getInstance().getConfig().getString("messages.blnotblock")));
                return true;
            }
            sql.sqlDeleteData("BLACKLIST", "UUID = '" + uuid + "' AND BlockedUUID = '" + unblockPlayer.getUniqueId() + "' AND BlockedPlayer = '" + unblockPlayer.getName() + "'");
            sender.sendMessage(ColorUtils.translateColorCodes(OgerLabs.getInstance().getConfig().getString("messages.blsuccessunblock")));
        } catch (Exception e) {
